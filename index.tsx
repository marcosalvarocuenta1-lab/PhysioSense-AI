...
const App = () => {
  ...
  // Inicializar GenAI
  // NOTA: La API Key se toma del entorno. Si falla, se muestra error en UI.
  ...
  const handleCharacteristicValueChanged = (event: any) => {
    ...
    // Asumimos formato: "val1,val2,val3,val4,val5" (puedes añadir \n en Arduino para mayor robustez)
  ...
  const generateAIReport = async () => {
    if (!process.env.API_KEY) {
      setAiReport("Error Crítico: No se detectó la API KEY de Gemini en el sistema. No se puede generar el reporte.");
      return;
    }
              ...
              <div className="md:w-2/3 flex flex-col h-full">
                ...
                {aiReport ? (
                  ...
                  <div className="h-full min-h-[250px] flex flex-col items-center justify-center text-slate-400 bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-xs opacity-70 mt-1">Requiere datos de sensores y nombre de paciente</p>
                  </div>
                )}
              </div>
            </div>
